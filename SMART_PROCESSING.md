...
Бот предлагает 2-5 релевантных тегов на английском языке (адаптируется под длину заметки).
...
### 3. Извлечение задач с датами и временем 📅✅ **НОВОЕ в v2.0!**

Автоматически находит конкретные действия (action items) в тексте **С СОХРАНЕНИЕМ временного контекста**.

**Пример:**
```
Ваш текст: "Завтра в 10:00 купить молоко. Сегодня вечером позвонить маме. 
...
- Относительные: "сегодня", "завтра", "послезавтра", "через 3 дня"
- Дни недели: "в понедельник", "во вторник"
- Абсолютные: "25.03", "25.03.2026"
- Периоды: "на следующей неделе"

**Извлечение времени:**
- Форматы: "в 10:00", "в 9:30", "вечером", "утром"

**Приоритеты задач:**
- 🔴 Высокий: "срочно", "важно", "ASAP"
- 🟡 Средний: обычные задачи
- 🟢 Низкий: "когда-нибудь", "не срочно"

### 4. Интеграция с Obsidian плагинами 🔌 **НОВОЕ в v2.0!**

Задачи теперь совместимы с популярными плагинами Obsidian:
- ✅ **Obsidian Tasks** - поддержка формата `📅 date ⏰ time`
- ✅ **Dataview** - метаданные `dates_mentioned` в frontmatter
- ✅ **Calendar** - поддержка Daily Notes формата
...
- [ ] Подготовить презентацию 📅 tomorrow #work
- [ ] Подготовить финансовую модель 📅 tomorrow #finance
...
**v1.0:** ~$0.0002 (0.02¢)  
**v2.0:** ~$0.00024 (0.024¢) **+20% из-за расширенного промпта**

### Месячные сценарии

| Использование | Заметок/день | Заметок/месяц | Стоимость v2.0/месяц |
|---------------|--------------|---------------|----------------------|
| Легкое | 10 | 300 | $0.07 |
| Среднее | 30 | 900 | $0.22 |
| Активное | 100 | 3000 | **$0.72** |
| Интенсивное | 200 | 6000 | $1.44 |

**Вывод:** Даже при активном использовании стоимость минимальна (~72 цента в месяц). Улучшенная функциональность с датами стоит дополнительных 20%!
...
### v2.0 - 17.02.2026
- ✨ **Извлечение дат и времени из задач**
- ✨ **Определение приоритетов задач**
- ✨ **Интеграция с Obsidian Tasks, Dataview, Calendar**
- ✨ **Адаптивное количество тегов** (2-3 для коротких заметок)
- ✨ **Метаданные `processing_version` и `dates_mentioned`**
- 📝 Улучшенный промпт LLM для временного контекста

### v1.0 - 17.02.2026
- ✨ Автоматические теги (3-5 штук)
- ✨ Краткое резюме
- ✨ Извлечение задач (без дат)
- ✨ Интерактивное редактирование
- ✨ Поддержка голосовых заметок

## 🔮 Будущие возможности (v3.0+)

Планируется в следующих версиях:

- **v3.0: Категоризация и связывание** - автоопределение типа заметки и поиск связей
- **v4.0: Контекст из Vault** - использование RAG для точной обработки
- **v5.0: Персонализация** - обучение на ваших предпочтениях
- **v6.0: Self-hosted LLM** - поддержка локальных моделей (Llama, Mistral)
